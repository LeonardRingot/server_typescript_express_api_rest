...
class UserModel {
  ...
  async create(u: User): Promise<User> {
    try {
      ...
      throw new Error(
        `Incapable de créer (${u.user_name}): ${(error as Error).message}
            `
      );
    }
  }
  ...
  async getMany(): Promise<User[]> {
    try {
      ...
      throw new Error(`Error at retriewing users ${(error as Error).message}`);
    }
  }
  ...
  async updateOne(u: User): Promise<User> {
    try {
      ...
      where id=$6
      RETURNING id, email, user_name, first_name, last_name, password`;
      const result = await connection.query(sql, [
        u.email,
        u.user_name,
        u.first_name,
